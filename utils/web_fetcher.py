...
YTDLP_PLATFORMS = {"Instagram", "TikTok", "YouTube", "Twitter/X", "Facebook", "Reddit",
                   "Pinterest", "Vimeo", "Twitch", "SoundCloud", "Spotify", "LinkedIn",
                   "Threads", "Tumblr", "Dailymotion", "VK", "OK.ru", "Snapchat", "CapCut"}
FOREIGN_SHORT_DOMAINS = {"t.co", "vm.tiktok.com", "vt.tiktok.com", "youtu.be", "fb.watch",
                         "instagr.am", "pin.it", "tmblr.co", "dai.ly", "on.soundcloud.com"}
...
class WebFetcher:
    ...
    def fetch_redirect_url(url, max_redirects=5):
        ...
        # ---- 国外平台直接交给 yt-dlp，不做本地重定向（反爬严格，直接透传更稳）----
        platform = UrlParser.get_platform(url)
        if platform in YTDLP_PLATFORMS:
            return url
        domain = UrlParser.get_domain(url)
        if domain in FOREIGN_SHORT_DOMAINS:
            # 短链接：先做一次轻量跳转，拿到最终URL后再给ytdlp
            try:
                resp = requests.get(url, headers=WebFetcher.headers, allow_redirects=True, timeout=8)
                return resp.url
            except Exception:
                return url
        ...
        if domain not in {"t.cn", "b23.tv", "xhslink.cn", "xhslink.com", "hy.fan"} | FOREIGN_SHORT_DOMAINS:
